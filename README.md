...
| [01_architecture.md](docs/01_architecture.md) | 总体架构图 + 设计原则 + LLM/程序边界总表 + 关键铁律 + 数据形态铁律 | 想理解整体架构与跨系统约束 |
| [02_app_data_and_modules.md](docs/02_app_data_and_modules.md) | 应用数据目录 + 配置分层 + 运行时快照 + 前/后端模块结构 + 模块职责边界 | 落地数据目录、配置和代码模块 |
...
| [13_agent_llm_io.md](docs/13_agent_llm_io.md) | PromptBuilder + CognitivePass I/O + 场景/结果/叙事 I/O 索引 + Dirty Flags | Agent LLM 节点提示词与结构化契约入口 |
...
| [16_agent_timeline_and_canon.md](docs/16_agent_timeline_and_canon.md) | TimeAnchor + WorldMainlineCursor + AgentSession + 过去线正史资格 | Agent 时间线与正史规则 |
| [17_agent_knowledge_model.md](docs/17_agent_knowledge_model.md) | KnowledgeEntry + access_policy + content schema + TruthGuidance + KnowledgeRevealEvent | Agent 知识模型与访问控制 |
| [18_agent_character_model.md](docs/18_agent_character_model.md) | CharacterRecord + BaseAttributes + BaselineBodyProfile + TemporaryCharacterState + 灵力显露倾向/运行时状态 | Agent 角色客观模型 |
| [19_agent_combat_and_skills.md](docs/19_agent_combat_and_skills.md) | Mana Combat Resolution + 对抗公式 + Skill Model 契约 | Agent 对抗解算与技能硬规则 |
