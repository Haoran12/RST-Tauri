...
ST 模式的目标是复刻 SillyTavern 的聊天体验：角色卡 V3、世界书、预设、聊天元数据与 Prompt 注入流程尽量兼容 SillyTavern 当前实现；RST 只在 Preset 与 API Provider 绑定关系上做有意偏离。
...
- **有意偏离点只有一类**：RST 解绑 Preset 与 API Provider。导入 / 导出尽量保留 ST 预设 JSON 原字段，但运行时不把 preset 绑定到 provider endpoint、key、model 或 connection profile。
