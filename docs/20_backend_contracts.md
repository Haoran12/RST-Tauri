...
Claude Code Interface 的请求格式复刻参考见 [CLAUDE_CODE_REQUEST_FORMAT.md](CLAUDE_CODE_REQUEST_FORMAT.md)；该文档是参考资料，不替代本文件定义的 `AIProvider` 抽象和日志边界。
...
`ChatRequest` 是 ST 模式、Agent PromptBuilder 与 ProviderRequestMapper 之间的中立请求形态；它不保存 API key、endpoint 或连接配置，这些只来自 `api_config_id` 对应的 API 配置：
...
pub enum ContentPart {
    Text { text: String },
    ImageRef { uri: String, mime_type: Option<String> },
