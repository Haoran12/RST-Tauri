...
| 8.3 | UI / UX 改进 | 🔄 | | 资源工作台与 API 配置页已接入；世界书 / 预设改为左侧文件选择器 + 条目/分区列表，左侧上下文栏滚动收敛到列表区域；主导航补齐 Regex，左侧上下文栏已接 ST 会话 / Agent 会话 / 角色卡 / 预设真实列表与点击动作；资源页细节和整体交互仍需继续打磨 |
...
| 8.14 | ST 会话级世界书与 User Persona 设置 | ✅ | 2026-05-06 | 会话 metadata 新增 `enabled_world_info` 多选与 `user_persona`；ST 会话侧栏三点菜单可编辑名称、世界书和 Persona Description，运行时组装读取这些字段 |
...
| 日期 | 更新内容 |
|------|----------|
| 2026-05-06 | 增加 ST 会话级世界书多选与 User Persona：会话文件保存 `enabled_world_info` / `user_persona`，侧栏三点菜单可编辑，会话发送与世界书匹配读取 Persona Description |
