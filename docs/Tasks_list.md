...
| 8.15 | ST 流式生成后台持久化与固定聊天页签 | ✅ | 2026-05-07 | ST stream 结束由后端按 `session_id + assistant_message_id` 写回会话并记录 stream chunks；前端生成状态持续到 stream end，ST Shell 改为固定聊天页签 + 工具页签，切到资源 / API / 日志页时聊天组件保持挂载 |
...
| 日期 | 更新内容 |
|------|----------|
| 2026-05-07 | 完成 ST 流式生成可靠性与固定页签布局：后端 stream task 累计响应后写回 ST 会话、补写 `llm_stream_chunks`，前端直到 stream end 才释放生成状态；App Shell 在 ST 工作区内保持聊天页签挂载，资源 / API / 日志作为工具页签切换 |
