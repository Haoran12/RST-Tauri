...
5. 集成 OpenAI Responses API、OpenAI Chat Completions API、DeepSeek Chat API。
...
5. 预设系统。
6. 多 API 支持（Anthropic Messages / Gemini / Claude Code Interface；本地模型或 Ollama 可作为扩展）。

### 阶段三：Agent 模式 — 数据模型层

1. SQLite 表结构 + 三层语义隔离（Layer 1 / Layer 3 / Trace）。
2. SceneModel + ManaField + PhysicalConditions 完整定义（Layer 1）。
3. KnowledgeEntry 体系（kind / subject / access_policy / subject_awareness / apparent_content）+ 访问派生索引表。
...
3. KnowledgeAccessProtocol（SQLite 派生索引预筛 + KnowledgeAccessResolver 裁剪，构建 AccessibleKnowledge）。
4. EmbodimentResolver（含灵觉 + 环境档位翻译）。
5. SceneFilter（含 observable_facets 计算 + WeatherPerception + ManaSignal）。
