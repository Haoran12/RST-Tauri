...
- **Simulation Core**（程序化优先）：场景维护、位置、可见性、身体状态、技能生成、结果规划硬边界。
...
2. **CharacterCognitivePass（人物认知与意图生成器）**：输入该角色可接触的 L2 场景、具身感官、可见设定与 prior L3；输出结构化心理活动、情绪、言行意图。它是强可见性限制节点。
3. **OutcomePlanner（结果规划器）**：输入 L1 场景真相、人物情绪与意图、技能契约/设定、结构化 DirectorHint；输出实际言行、交互/对抗结果、状态更新计划候选。它可以 God-read，但不能直接提交状态。
4. **SurfaceRealizer（叙事文本输出）**：输入 NarrationScope 限制后的场景、人物心理/情绪摘要、实际言行、交互结果、文风/格式要求；输出自由文本叙事。它是强叙事可见性限制节点。
...
- `knowledge_revealed`：Tier A/B 角色本回合获得新可见 Knowledge。
...
- primary target、盟友/守护者、领域/被动技能拥有者、被波及者都可以成为候选反应者，但必须能通过本回合 `FilteredSceneView` / `VisibleEventDelta` 感知到威胁。
...
12a. 程序从用户扮演 intent、场景事件、NPC IntentPlan 中打开 ReactionWindow
     ...
     - 援护者/伙伴/被动领域按 SkillEffectContract 与可见性判断资格
...
13. OutcomePlanner（结果规划器，God-read，每回合默认最多 1 次）：
    ...
    d. EffectValidator 校验资源/位置/技能契约/数值/可见性/GodOnly 规则；合法硬效果保留，非法或越界硬效果转入 blocked_effects 或 soft_effects，不反复调用 LLM 修复
    e. 按 NarrationScope 派生 visible_facts 白名单
...
| 13d-13e | 记录 EffectValidator 裁剪结果、资源消耗、命中、visible_facts、blocked_effects / soft_effects | 结果规划异常 |
...
8. **Narrative Fact Check Rule** - SurfaceRealizer 输出的叙事文本中提及的具体事实必须 ⊆ `OutcomePlan.visible_facts` 白名单；不可引入新事实（位置/伤势/状态变化等）。修辞描写不计。
