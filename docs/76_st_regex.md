...
| `preset` | Preset Scripts | 当前生成预设的扩展字段 `regex_scripts` | 预设名必须在 `preset_allowed_regex[apiId]` 中 |
...
- 预设内嵌脚本存放在 preset 扩展字段 `regex_scripts`。首次遇到时提示用户允许；允许后把当前 preset 名写入 `preset_allowed_regex[apiId]`。
