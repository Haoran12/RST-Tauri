...
- `src/components/layout/AppLayout.vue` 已实现左侧导航、上下文侧栏、主工作区、右侧检查面板的基础布局切分。
- `src/components/layout/AppNav.vue` 已实现一级导航入口。
- `src/components/layout/ContextList.vue` 与 `InspectPanel.vue` 已提供 route-aware 只读摘要和基础导航数据，但尚未覆盖所有业务页的深度交互。
...
| `agent` | 目标 store，承载 Agent World、会话、运行状态、主线光标、Trace 入口摘要；当前尚未建立 |
| `agentWorldEditor` | 目标 store，承载 editor snapshot、draft、validation result、impact summary、commit result；当前尚未建立 |
