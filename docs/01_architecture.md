...
- **Truth ≠ Accessible Truth** — 客观真相只有编排器、结果规划、验证可读；角色 LLM 永远经过可见性过滤。
- **Single Source of Visibility** — 所有可见性业务判断由 VisibilityResolver 集中处理；SQLite 索引只做候选预筛，禁止散落在 prompt builder 或业务代码中。
...
`KnowledgeEntry.content` 必须包含核心结构化字段（用于程序判断、可见性、检索），可选包含 `summary_text` 等自由文本辅助字段（仅供 LLM 阅读理解）。详见 [10_agent_data_model.md](10_agent_data_model.md) 的 KnowledgeEntry 章节。
...
Agent 模式不再把"LLM"视为单一权限主体。每个 LLM 节点必须声明输入域、输出域、可见性权限和提交权限。
...
| CharacterCognitivePass（人物认知与意图生成器） | 程序派生的该角色 L2 视图 + prior L3；字段值可含 `llm_readable` 文本 | 结构化心理活动、情绪、言行意图；字段值可含 `llm_readable` 文本 | 严格受 VisibilityResolver 过滤；不得读取 L1 原始对象或 GodOnly 知识 |
...
| SurfaceRealizer（叙事文本输出器） | NarrationScope 派生的 SceneNarrativeView、角色心理/情绪摘要、实际言行、交互/对抗结果、文风/格式/叙事倾向 | 面向用户的自由文本叙事 | 不得突破 NarrationScope / visible_facts；不得引入新事实 |

权限规则：

- **God 读取权限不等于提交权限**：SceneStateExtractor / OutcomePlanner 即使读取 L1，也只能输出严格 schema 的候选 delta / plan；写库只由程序提交。
- **受限 LLM**：CharacterCognitivePass 和 SurfaceRealizer 是主要防泄露对象，必须只接收过滤后的输入。
- **程序验证永远在提交前**：任何 LLM 产出的状态变化都必须经过 schema 校验、一致性校验、可见性校验与 StateCommitter。
...
| 可见性候选索引 | 程序 + SQLite | `visibility` JSON 是权威结构；`known_by` / `scope` 派生索引只用于缩小候选集，最终仍由 VisibilityResolver 判定 |
| 场景过滤 + visible_facets | 程序 | 全程结构化 |
| KnowledgeAccess | 程序 | 先用 SQLite 派生索引预筛候选，再调用 VisibilityResolver 裁剪为 AccessibleKnowledge |
...
| 物理公式与硬约束（资源/位置/技能数值/可见性） | 程序 | 不依赖自由文本；可用于校验、裁剪或阻止候选效果 |
...
2. **VisibilityResolver 永不调 LLM**：可见性判断必须确定性；数据库索引只服务查询性能，不承担最终判定。
3. **LLM 输出必须严格 schema**：优先依赖 Provider 的 structured output / tool schema；仅在无强 schema 能力时退化到 JSON mode + schema 校验 + 重试 / 程序容错。
4. **受限 LLM 不读真相**：CharacterCognitivePass 和 SurfaceRealizer 只读过滤后的输入；SceneStateExtractor / OutcomePlanner 的 God-read 权限必须显式声明。
5. **数值字段不让受限 LLM 直出**：信念/情绪变化用离散级别，由程序映射为数值；对抗解算数值结果必须可被程序公式校验。
6. **God 读取不等于提交权限**：结果规划与场景提取 LLM 只产出候选 JSON，最终状态写入必须由程序校验并提交。
...
8. **叙事不引入新事实**：SurfaceRealizer 受 NarrationScope 派生的 visible_facts 白名单约束，由 NarrativeFactCheck 强制。
9. **PromptBuilder 是 Agent LLM 调用唯一入口**：静态提示词只写节点契约并版本化；动态部分只传对应 `*Input` schema JSON；不得把日志、隐藏事实或临时自然语言说明绕过类型系统塞进 prompt。
10. **日志不驱动业务**：Agent Trace 和运行 Logs 只用于观察、调试、审计、回放定位；不得参与程序判断、检索、可见性或 LLM prompt 组装。
...
`api_configs/` 是全应用共享的 AI Provider 配置池。第一版 Agent 模式不另建一套 Provider 配置，而是从该配置池中为四类 Agent LLM 节点分别选择配置；具体绑定关系存于 Agent profile / World settings，见 [20_backend_contracts.md](20_backend_contracts.md)。
...
| 模块 | 唯一职责 | 禁止做的事 |
|---|---|---|
| `knowledge::store` | KnowledgeEntry 的 CRUD；同事务维护可见性派生索引 | 不做可见性判断，不读 Layer 3 |
| `knowledge::visibility` | 给定 (entry, character, context) → bool | 严禁调 LLM；不读 Layer 3 belief；不修改任何状态；不依赖 SQL 结果作为最终判定 |
| `knowledge::access` | 用 SQLite 派生索引预筛候选，并经 VisibilityResolver 构建 AccessibleKnowledge | 不调 LLM，不修改 belief；不绕过 VisibilityResolver |
