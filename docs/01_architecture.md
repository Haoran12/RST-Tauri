...
- **World Editor Is Structured Authoring** — Agent 世界编辑器只写结构化 Layer 1 / World rules 数据；它不是 LLM 节点，不参与回合推理，不直接改 Layer 2 / Layer 3 / Trace，运行中提交必须在 World paused 且无活跃回合 / LLM 调用时进行。
- **Structured Text Editor Is UI Hygiene** — 结构化文本编辑器只负责 Plain / JSON / YAML 文本编辑、缩进矫正、括号 / 引号诊断和保存前解析提示；它不解释世界语义，不替代 ST 兼容规则、Regex 编译、Agent schema validator、KnowledgeAccessResolver 或 World Editor paused-only 门禁。
- **Prompt Contracts Are Control Plane** — Agent LLM 提示词只定义节点身份、权限、任务和输出要求；世界事实只能来自本次结构化 input。
- **Prompt Budget Is Deterministic** — Agent prompt 的 token 估算、P0/P1/P2/P3 重要性排序、压缩、裁剪和多人物 CognitivePass 分层调度都由程序执行；不得让 LLM 自行决定哪些事实可丢弃，也不得通过预算裁剪绕过权限边界。
...
ST 模式允许用户消息包含图片 / PDF 附件，但附件必须遵守独立数据边界：

- 本地源文件保存在 `./data/` 下的附件库，不内嵌到聊天 JSON。
- 聊天记录只保存稳定 `attachment_id` 引用和必要展示元数据。
- Provider 传输层再把同一附件映射为 inline bytes、Provider Files API 句柄或兼容内容块。

禁止把附件静默降级为“假装等价的文本消息”再发送给不支持多模态的 Provider。若需要 OCR / 文本抽取，必须作为显式工作流或工具结果，而不是伪装成原附件仍被原生发送。
...
2. **用户发言是分级输入权，不是无条件写入权**：用户可提出角色言行、心理、场景旁白、导演提示和元命令；越接近所扮演角色与本回合呈现，权限越强，越接近 canonical Truth、隐藏 Knowledge、硬状态、硬数值和他人内心，越必须降级为候选、引导、假设或歧义，并由程序校验、确认或阻止。
3. **KnowledgeAccessResolver 永不调 LLM**：Knowledge 访问权限判断必须确定性；数据库索引只服务查询性能，不承担最终判定。
4. **LLM 输出必须严格 schema**：优先依赖 Provider 的 structured output / tool schema；仅在无强 schema 能力时退化到 JSON mode + schema 校验 + 重试 / 程序容错。
5. **受限 LLM 不读真相**：CharacterCognitivePass 和 SurfaceRealizer 只读过滤后的输入；SceneInitializer / SceneStateExtractor / OutcomePlanner 的 God-read 或公开上下文读取权限必须显式声明。
6. **数值字段不让受限 LLM 直出**：基础属性与 `mana_power` 存储/计算可用 f64，但受限 LLM 只读 tier / delta / expression_assessment / pressure_hints / descriptors / constraints；灵力显露只能区分持久倾向与当前运行时状态，不让 LLM 直出 display/pressure 倍率；信念/情绪变化用离散级别，由程序映射为数值；对抗解算数值结果必须可被程序公式校验。
7. **God 读取不等于提交权限**：场景初始化、结果规划与场景提取 LLM 只产出候选 JSON，最终状态写入必须由程序校验并提交。
8. **反应窗口有界**：主动行动可打开一次 ReactionWindow；窗口内只收集合法 ReactionIntent，不即时递归结算；默认 `no_reaction_to_reaction = true`、`one_reaction_per_character_per_window = true`。
9. **叙事不引入新事实**：SurfaceRealizer 受 NarrationScope 派生的结构化 narratable_facts 白名单约束，由 NarrativeFactCheck 强制。
10. **PromptBuilder 是 Agent LLM 调用唯一入口**：静态提示词只写节点契约并版本化；动态部分只传对应 `*Input` schema JSON；不得把日志、隐藏事实或临时自然语言说明绕过类型系统塞进 prompt。
11. **ST 多模态附件三层分离**：本地源文件、聊天记录引用、Provider 上传缓存 / 传输内容块必须分层持有；远端 `file_id` / `file_uri` 只是运行时缓存，不能替代本地真源。
11. **Prompt 预算是确定性控制面**：每次 Agent LLM 请求都由 PromptBuilder / InputAssembly 估算输入 token；达到软上限后按 P0/P1/P2/P3 规则压缩和裁剪，超过可配置最大上下文时必须继续裁剪到上限内；未执行完整 CognitivePass 的人物只能复用意图、模板意图、MinorActorSlot 外显补全或 crowd behavior，不能凭空更新 L3。
12. **日志不驱动业务**：Agent Trace 和运行 Logs 只用于观察、调试、审计、回放定位；不得参与程序判断、检索、访问控制或 LLM prompt 组装。
13. **地点推断不固化弱假设**：`parent_id`、`LocationSpatialRelation`、`LocationEdge` 与结构化 RegionFact 才是地点真相；同父级、同自然地理带、同层级、名称相似、LLM 判断等只能产生带置信度的 `ProximityHint` / `SceneAssumption`，不能自动写入空间关系、路线边或地区事实。
14. **配置不在热路径做 IO**：数值阈值、日志清理上限、运行预算等可配置项不得散落为业务硬编码；程序启动 / World 打开时合并、校验并发布不可变 `RuntimeConfigSnapshot`，一回合内所有 Resolver / Filter / RetentionManager 只读该快照，不读文件或临时查询配置表。
15. **回合内工作副本不等于持久状态**：SceneInitializer / SceneStateExtractor / OutcomePlanner 产出的 draft / delta / plan 只能先应用到本回合 `TurnWorkingState`，供后续派生、验证和叙事组装读取；只有 `StateCommitter` 可在单个 SQLite 写事务中把通过校验的 L1 / L3 / Knowledge / Trace 变更提交为持久状态。
16. **作者编辑不伪装成运行回合**：World Editor 的作者提交写入独立 editor commit journal，不写 `world_turns` 或 `state_commit_records`，也不伪造 `scene_turn_id`；它可以修改 canonical Truth，但必须通过 paused-only 门禁、schema 校验、影响分析、派生索引同步和单事务提交。
17. **文本格式化不等于业务合法**：Structured Text Editor 的 JSON / YAML 解析和 Plain 缩进矫正只能证明文本形态可读；ST content 和 prompt 正文可以用 JSON / YAML 写成给 LLM 阅读的结构化文本，但程序仍按字符串参与提示词和注入，不把其中 key 当业务规则；Regex 仍需编译校验，Agent structured content 仍需业务 schema、权限、影响分析和提交门禁。
