...
- 新 World 拥有独立的 `world.sqlite`、`world_base.yaml`、`assets/`、会话列表、主线光标、Trace 和后续运行 Logs。
...
4. **World 规则配置**：`./data/worlds/<world_id>/world_base.yaml`，保存 `AttributeTier` 边界、`AttributeDelta` 桶、`CombatOutcomeTier` 桶、压制破绽阈值、环境档位阈值等会改变世界物理刻度的规则。
