...
应用数据根目录固定为应用所在路径下的 `./data/`。默认不得写入 `AppData`、`Application Support`、`~/.config` 等系统用户数据目录，除非用户把应用本体安装在对应磁盘且数据仍位于应用自己的目录内。理由是让用户可以直接复制、备份、同步和检查完整数据。

### 1.1 通用规则

- 所有用户可迁移数据必须位于 `./data/` 或用户显式选择的数据根目录下；Windows 下若应用本体不在 `C:`，`RST_DATA_DIR` 等覆盖路径也不得指向 `C:`。
- WebView 产生的 localStorage、IndexedDB、cookies、HTTP cache 等运行数据也必须落在 `./data/webview/`，不能使用 Tauri / WebView 默认的系统用户数据目录。
...
```
./data/
├── chat_attachments/ # ST 聊天附件源文件与派生缓存
├── lores/          # 世界书
├── presets/        # 预设（与 API 配置解耦；每个 JSON 含六类 section）
│   ├── Default.json
│   └── <preset_name>.json
├── chats/          # 聊天记录
├── characters/     # 角色卡 V3
...
└── webview/        # Tauri WebView localStorage / cache / cookies / IndexedDB
```

ST 模式的聊天记录是文本会话数据，不承担 Agent 世界状态演化职责；删除 / 编辑消息不强制触发世界回滚约束。

`chat_attachments/` 用于保存 ST 模式用户上传的图片 / PDF 附件。聊天 JSON 只保存 `attachment_id` 引用，不内嵌 base64。附件的本地存储、聊天记录格式和 Provider 发送链路见 [77_st_multimodal_attachments.md](77_st_multimodal_attachments.md)。

**API 配置与 ST 资源独立于会话**：
- API 配置（Provider、endpoint、model、鉴权）存储在 `./data/api_configs/`
- 预设（采样参数、提示词模板）以单个 `PresetFile` 存储在 `./data/presets/<name>.json`，默认保证存在 `Default.json`
- 世界书、ST 会话角色卡绑定、角色卡世界书绑定、聊天 world_info metadata、会话级世界书多选和 User Persona 存储在 `./data/lores/`、`./data/characters/`、`./data/chats/`
- 图片 / PDF 聊天附件存储在 `./data/chat_attachments/`，会话消息只保留稳定引用
...
- 聊天附件的持久化路径必须使用生成的 `attachment_id`，不能直接拼接用户文件名。
- `original_filename` 只作为展示提示，不能作为真实文件名或目录名参与持久化。
- 用户粘贴或输入远程 URL 作为附件时，必须先下载 / 镜像到 `./data/chat_attachments/`，再允许其进入聊天记录。
- Provider 远端 `file_id` / `file_uri` 只可作为运行时上传缓存，不能替代本地源文件。
...
若用户希望基于当前世界状态显式走出“另一条世界分支”，第一版不在同一 World 内引入第二条 canonical timeline，而是通过 `/fork` **复制整个 World**：

- `/fork` 会创建新的 `world_id` 与新的 `./data/worlds/<new_world_id>/` 目录。
- 新 World 拥有独立的 `world.sqlite`、`world_argument.yaml`、`assets/`、会话列表、主线光标、Trace 和后续运行 Logs。
- 复制完成后，新旧 World 之间不共享 canonical Truth、不共享 `WorldMainlineCursor`，也不共享后续回合提交。
- 因此，`/fork` 是“复制世界并进入副本”，不是“在当前 World 内开启平行主线”。
...
`/fork` 不属于回滚，也不属于当前会话的 `MetaCommand` 状态推进。它更接近 World 级复制操作：

- 成功执行后，当前工作上下文切换到新 World / 新 Session 入口。
- 不改写源 World 的任何 canonical turn、session_turn、trace 或日志。
- 默认复制当前 World 的权威数据库与资源；必要时可在实现中选择是否裁剪全局索引事件，但不得让新 World 继续引用旧 World 的可变状态。
...
3. **Provider 协议契约**：打包随应用发布的 `config/llm_api_contracts.json`，保存一等 Provider 的字段白名单、协议差异、结构化输出、多模态输入能力与 RST 级适配策略。
4. **World 规则配置**：`./data/worlds/<world_id>/world_argument.yaml`，保存 `AttributeTier` 边界、`AttributeDelta` 桶、`CombatOutcomeTier` 桶、压制破绽阈值、环境档位阈值等会改变世界物理刻度的规则。
5. **运行期 UI 草稿**：设置界面编辑时先进入 draft，只有通过校验并保存后才发布新快照。
...
```yaml
# ./data/worlds/<world_id>/world_argument.yaml
schema_version: 1
attribute_rules:
  tier_thresholds:
    mundane: [0, 200]
...
- `LlmContractLoader` 只在应用启动、打包内置资源更新、显式 reload 或开发调试动作时读取 `config/llm_api_contracts.json`。
- `LlmContractCompiler` 把 `llm_api_contracts.json` 编译为只读 `LlmApiContractsSnapshot`：协议字段白名单、`input_capabilities`、`multimodal_attachment_policy`、`model_capability_snapshot_shape` 和 provider 适配辅助索引。
- `ConfigRegistry` 以 `Arc` / 只读引用发布当前 `RuntimeConfigSnapshot`、`WorldRulesSnapshot` 与 `LlmApiContractsSnapshot`；Resolver、Filter、RequestAssembler、RetentionManager、CapabilityResolver、ProviderRequestMapper 只接收快照引用，不直接依赖文件路径。
- `ProviderContractCache` 负责按当前 API 连接维度缓存已编译的 provider 契约视图。cache key 至少包含 `api_config_id + provider_kind + protocol_kind + model + base_url`；Claude Code Interface 还应包含 `provider_variant`。切换 API 配置或模型时只重建对应 cache entry，不全量重读文件。
...
ST 模式请求组装同样遵守“配置不在热路径做 IO”：`RequestAssembler`、`AttachmentResolver`、`CapabilityResolver`、`ProviderRequestMapper` 不得在每次请求时重新打开 `config/llm_api_contracts.json`。它们只读取 `LlmApiContractsSnapshot` 与 `ProviderContractCache`。
...
pub struct LlmApiContractsSnapshot {
    ...
    pub first_class_protocols: Vec<String>,
    pub provider_contracts: serde_json::Value,   // 编译后的协议契约与 capability 索引
    pub multimodal_policy: serde_json::Value,    // 编译后的 RST 附件策略
}
```
...
│   │   └── sqlite_store.rs
│   ├── config/
│   │   ├── loader.rs       # app_runtime.yaml / world_argument.yaml 加载
│   │   ├── validator.rs    # schema version、范围、单调性校验
│   │   ├── llm_contracts.rs # llm_api_contracts.json 加载、编译、ProviderContractCache
│   │   └── registry.rs     # RuntimeConfigSnapshot / WorldRulesSnapshot / LlmApiContractsSnapshot 发布
...
| `config::llm_contracts` | 加载并编译 `config/llm_api_contracts.json`，发布 `LlmApiContractsSnapshot`，维护 `ProviderContractCache` | 不直接发送请求；不读取聊天资源；不在每次请求热路径重读 JSON 文件 |
| `st::capability_resolver` | 按 `api_config_id + provider + model + base_url/variant` 读取已编译契约视图，判断结构化输出、多模态输入与默认 transport | 不打开配置文件；不私自覆盖主文档约束；不对不支持的能力做静默 OCR / PDF 文本降级 |
| `st::provider_request_mapper` | 依据 `ProviderContractCache` 白名单和 capability 把中立 `ChatRequest` 映射为具体 Provider 请求 | 不手写散落 provider 字段规则；不绕过 `llm_api_contracts.json` 白名单；不做额外文件 IO |
